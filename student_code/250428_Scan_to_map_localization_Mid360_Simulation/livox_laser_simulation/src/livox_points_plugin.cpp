...
void convertDataToRotateInfo(const std::vector<std::vector<double>> &datas, std::vector<AviaRotateInfo> &avia_infos) {
    ...
    for (int i = 0; i < datas.size(); ++i) {//xxx
        auto &data = datas[i];
        if (data.size() == 3) {
...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    std::string file_name = "/home/byy/catkin_ws/src/livox_laser_simulation/scan_mode/mid360.csv";
    ...
    // 替换话题中的变量
    std::string topic_name = curr_scan_topic;
    //boost::replace_all(topic_name, "${vehicle_name}", "iris_" + std::to_string(uav_num));

    ROS_INFO_STREAM("ros topic name:" << topic_name);
    ...
    switch (publishPointCloudType) {
        case SENSOR_MSG_POINT_CLOUD:
            rosPointPub = rosNode->advertise<sensor_msgs::PointCloud>(topic_name, 5);
            break;
        case SENSOR_MSG_POINT_CLOUD2_POINTXYZ:
        case SENSOR_MSG_POINT_CLOUD2_LIVOXPOINTXYZRTL:
            rosPointPub = rosNode->advertise<sensor_msgs::PointCloud2>(topic_name, 5);
            break;
        case LIVOX_ROS_DRIVER_CUSTOM_MSG:
            rosPointPub = rosNode->advertise<livox_ros_driver2::CustomMsg>(topic_name, 5);
            break;
        default:
            break;
    }
